...
import {
  getBikeFiltersFromSparify,
  getShopByBikeItemBySlug,
  shopByBikeItems,
} from "@/lib/shop-by-bike-data";
import ExpandableFilterList from "@/components/expandable-filter-list";

type ShopByBikeDetailPageProps = {
  params: Promise<{ bikeSlug: string }>;
};
...
export function generateStaticParams() {
  return shopByBikeItems.map((item) => ({ bikeSlug: item.slug }));
}

export default async function ShopByBikeDetailPage({ params }: ShopByBikeDetailPageProps) {
  const { bikeSlug } = await params;
  const bike = getShopByBikeItemBySlug(bikeSlug);

  if (!bike) {
    notFound();
  }

  const filters = await getBikeFiltersFromSparify(bike.searchTerm);
      ...
      <div className="mx-auto grid max-w-7xl gap-5 px-4 sm:px-6 lg:grid-cols-[260px_1fr] lg:px-8">
        ...
        <div>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">{bike.brand}</p>
              <h1 className="mt-1 text-3xl font-semibold tracking-tight text-zinc-900">{bike.model}</h1>
            </div>
          ...
          <div className="grid border border-zinc-300 bg-white sm:grid-cols-2 xl:grid-cols-4">
            {productImagePool.map((image, index) => {
              ...
              return (
                <article key={`${bike.slug}-${index}`} className="relative border-b border-r border-zinc-300 p-2.5">
                  <div className="flex h-48 items-center justify-center">
                    <img src={image} alt={`${bike.model} product`} className="h-44 w-full object-contain" />
                  </div>

                  <h3 className="line-clamp-2 min-h-[42px] text-base font-semibold text-zinc-900">
                    {bike.model} Product {index + 1}
                  </h3>
          ...
          <div className="mt-5">
            <Link href="/shop-by-bike" className="text-sm font-medium text-zinc-700 hover:underline">
              ← Back to Shop By Bike
            </Link>
          </div>
        </div>
      </div>
