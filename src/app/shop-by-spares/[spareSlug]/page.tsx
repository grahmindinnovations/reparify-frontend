...
import {
  getShopBySparesItemBySlug,
  getSparesFiltersFromSparify,
  shopBySparesItems,
} from "@/lib/shop-by-spares-data";
import ExpandableFilterList from "@/components/expandable-filter-list";

type ShopBySparesDetailPageProps = {
  params: Promise<{ spareSlug: string }>;
};
...
export function generateStaticParams() {
  return shopBySparesItems.map((item) => ({ spareSlug: item.slug }));
}

export default async function ShopBySparesDetailPage({ params }: ShopBySparesDetailPageProps) {
  const { spareSlug } = await params;
  const spare = getShopBySparesItemBySlug(spareSlug);

  if (!spare) {
    notFound();
  }

  const filters = await getSparesFiltersFromSparify(spare.searchTerm);
      ...
      <div className="mx-auto grid max-w-7xl gap-5 px-4 sm:px-6 lg:grid-cols-[260px_1fr] lg:px-8">
        ...
        <div>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">{spare.category}</p>
              <h1 className="mt-1 text-3xl font-semibold tracking-tight text-zinc-900">{spare.name}</h1>
            </div>
          ...
          <div className="grid border border-zinc-300 bg-white sm:grid-cols-2 xl:grid-cols-4">
            {productImagePool.map((image, index) => {
              ...
              return (
                <article key={`${spare.slug}-${index}`} className="relative border-b border-r border-zinc-300 p-2.5">
                  <div className="flex h-48 items-center justify-center">
                    <img src={image} alt={`${spare.name} product`} className="h-44 w-full object-contain" />
                  </div>

                  <h3 className="line-clamp-2 min-h-[42px] text-base font-semibold text-zinc-900">
                    {spare.name} Product {index + 1}
                  </h3>
          ...
          <div className="mt-5">
            <Link href="/shop-by-spares" className="text-sm font-medium text-zinc-700 hover:underline">
              ← Back to Shop By Spares
            </Link>
          </div>
        </div>
      </div>
